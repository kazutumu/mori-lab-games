...
export default function SailingM1Game({onClear}:Props){
  ...
  const toggleSail=useCallback(()=>{
    ...
    setMessage(sailingRef.current?"出航しました。風向きに帆を合わせ、三つの光を集めます。":"帆をゆるめました。接岸するときは6ノット以下まで減速します。");
  },[]);
  ...
  useEffect(()=>{
    ...
    const animate=()=>{raf=window.requestAnimationFrame(animate);const delta=Math.min(clock.getDelta(),.04);elapsed+=delta;oceanMaterial.uniforms.uTime.value=elapsed;
      ...
      const dockDistance=Math.hypot(boat.position.x-DOCK.x,boat.position.z-DOCK.z);if(!finishedRef.current&&collected.size===FRAGMENTS.length&&dockDistance<3.2){if(currentSpeed<=4.2){finishedRef.current=true;sailingRef.current=false;setSailing(false);setFinished(true);setComplete(true);celebrating=true;const seconds=Math.max(1,Math.round(missionTime));const grade=stabilityRef.current>=82&&seconds<=70?"星3・静かな名航海":stabilityRef.current>=58?"星2・安全航海":"星1・手紙を届けた航海";setFinalTime(seconds);setRating(grade);setMessage("桟橋へ接岸しました。ミナが朝の手紙を島へ届け、バンザイしています！");if(!rewarded.current){rewarded.current=true;onClearRef.current();}}else{setMessage("桟橋進入路の浅瀬が船を減速させます。6ノット以下で接岸できます。");}}
  ...
  return <div className="m1-game">
    ...
    </div><div className="m1-trim"><label htmlFor="m1-trim-range">帆の向き <strong>{trim > 0 ? "+" : ""}{trim}°</strong></label><input id="m1-trim-range" type="range" min="-45" max="45" step="5" value={trim} onChange={event=>{const value=Number(event.target.value);trimRef.current=value;setTrim(value);}}/><span>風向き {wind > 0 ? "+" : ""}{wind}°へ近づけると速く、安定します</span></div><div className="m1-note"><span>目的：風のかけら3つ → 桟橋進入路で自然減速 → 6ノット以下で接岸</span><span>← → / Space / C</span></div></div>
